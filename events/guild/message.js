...
module.exports = async (client, message) => {
    ...
    if (cmd.length === 0){
      ...
      if(matchedPrefix.includes(client.user.id)){
        ...
        .setAuthor(`Getting Stared | Violent Music`, client.user.displayAvatarURL({dynamic: true}), config.links.opmusicinv)
          ...
          if(guildData.djRoles.length !== 0){
            ...
            if(!isdj && !message.member.permissions.has("ADMINISTRATOR")) {
              if(!player.queue.current) {
                ...
                .setColor(ee.wrongcolor)
                .setAuthor(`${message.author.tag} - DJ Only Command`, message.author.displayAvatarURL())

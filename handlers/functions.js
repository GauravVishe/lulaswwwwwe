  ...
  collector.on("collect", async (i) => {
    try {
      ...
      if (i.customId == "next") {
        ...
        const idk = await message.channel.send({embeds: [emem]})
        const collector = message.channel.createMessageCollector(m => m.author.id === message.author.id, { time: 10000 });
        collector.on("collect", async msg => {
          if(isNaN(msg.content) || Number(msg.content) > embeds.length) {
            ...
            message.channel.send({embeds: [eoe]}).then(msg => setTimeout(() => { msg.delete() }, 3000))
            collector.stop()
          } else {
            msg.delete()
            try { idk.delete() } catch {/** */}

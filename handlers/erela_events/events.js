...
module.exports = (client, message) => {
  ...
  .on("trackStart", async (player, track, message) => {
    try {
      ...
      embed.setAuthor(`${track.requester.tag} - Now Playing`, track.requester.displayAvatarURL({dynamic: true}))
  ...
  .on("queueEnd", async (player) => {
    ...
    .setDescription(`I have played every song in queue, Queue more song to keep the party going.Enjoying music with me? Consider Inviting me on your server.`);
    try{
      client.channels.cache.get(player.textChannel).send({embeds: [embed]})
    } catch {/* */}
    return ;
